...
# base: ./DESIGN.md
# brand: Loop
# brand-primary: unchanged hue (Path B), saturation +20%, #5238ff
# generated: 2026-07-01
version: alpha
name: loop-design-system-bold
brand: Loop
description: Loop's design system (bold variant) — a louder, higher-contrast reading of the Stripe-inspired reference. Primary saturated +20%, much larger border radii, an expanded 1.618 (golden ratio) type scale on the display/heading tier, stronger multi-layer shadows, and richer surface tints. Suited to D2C / marketing / creative-led products.
source: reverse-engineered via VoltAgent/awesome-design-md (design-md/stripe/DESIGN.md), Path B — primary hue kept as-is, saturation increased

colors:
  primary: "#5238ff"
  primary-deep: "#3121ce"
  primary-press: "#252196"
  primary-soft: "#645cff"
  primary-bg-subdued-hover: "#b3b3ff"
...
Loop's design language (bold variant) opens with the gradient mesh, turned up. A wide horizontal band of pastel cream, sherbet orange, lavender, saturated electric indigo, and ruby pink occupies the upper third of nearly every marketing page — the brand's instantly-recognizable atmospheric backdrop, pushed louder than the reference. Type and product UI mockups float above it on `{colors.canvas}` (white), with the gradient acting as both decoration and visual anchor. The lower portion of the page returns to white, with feature explanations on `{colors.canvas-soft}` (now a distinctly saturated cool blue tint) and dashboard product mockups composited as faux IDE/console panels in deep navy.

The color system has two primary roles. **Indigo** (`{colors.primary}` — `#5238ff`, saturated +20% from the reference) is the brand's signature CTA color, used sparingly: one filled pill per band. **Deep navy** (`{colors.ink}` — `#0d253d`) is the universal body text color and the fill of dashboard mockups, the featured pricing tier, and the dark-app surfaces on the dashboard track. Ruby (`{colors.ruby}`) and magenta (`{colors.magenta}`) appear inside the gradient mesh and as accent dots in product UI mockups; they are not used as button colors.

Typography is built around **Sohne** at weight 300 with negative letter-spacing — the brand's editorial-density display signature, expanded here to a dramatic 1.618 (golden ratio) scale on the display/heading tier, so headline sizes leap between steps. Body sizes use 0 tracking, unchanged from the reference; tabular caption sizes (where money and numerics matter) use the OpenType `tnum` feature plus a tightening -0.36 to -0.42px tracking. The `ss01` stylistic set is enabled across all roles.

**Key Characteristics:**
- Gradient-mesh backdrop on every marketing hero, at increased saturation — cream/orange/lavender/indigo/ruby washed across the upper third of the page, louder than the reference.
- Single-indigo CTA hierarchy: filled `{colors.primary}` pill is the only filled button on marketing surfaces.
- Sohne thin (weight 300) display tier on an expanded 1.618 golden-ratio scale — dramatic, editorial headline jumps up to 72px.
...
- Pill-shaped buttons (`{rounded.pill}` 9999px) with tight `8px 16px` padding — short, decisive, transactional; all other radii (cards, inputs, tags) are expanded to an 8–32px scale for a rounder, more expressive silhouette.
- Stronger multi-layer shadows: Level 1 and Level 2 elevation both gain wider blur and spread for a more dramatic lift than the reference.
- Cream-band feature cards (`{colors.canvas-cream}`, deepened to a richer gold-cream) introduce a bolder warm interlude between blue/white sections without breaking the brand's chromatic logic.
...
### Brand & Accent
- **Indigo** (`{colors.primary}` — `#5238ff`): The brand's signature CTA color, saturated +20% from the `#533afd` reference for a louder feel. Filled-pill button, link emphasis, gradient anchor.
- **Indigo Deep** (`{colors.primary-deep}` — `#3121ce`): A deeper indigo used in gradient mid-stops and as the press-state warmer alternative.
- **Indigo Press** (`{colors.primary-press}` — `#252196`): Pressed-state lift of the primary.
- **Indigo Soft** (`{colors.primary-soft}` — `#645cff`): A lighter indigo used in product-UI accents and chart highlights.
- **Indigo Subdued** (`{colors.primary-bg-subdued-hover}` — `#b3b3ff`): Pale indigo fill used as soft tag background.
...
- **Canvas Soft** (`{colors.canvas-soft}` — `#d8e6f7`): Distinctly saturated cool blue tint used on feature bands beneath the gradient hero — a visible color block, not a barely-tinted white.
- **Canvas Cream** (`{colors.canvas-cream}` — `#eed7a0`): Deepened gold-cream used as a feature-band fill — a bolder chromatic interlude than the reference.
...
When Sohne is unavailable, fall back to **SF Pro Display** at thin weights, then system-ui. For maximum brand fidelity, **Inter** (open-source) at weight 300 with `font-feature-settings: "ss01"` and the expanded negative tracking on display sizes approximates the rhythm closely.

### Hierarchy

| Token | Size | Weight | Line Height | Letter Spacing | Use |
|---|---|---|---|---|---|
| `{typography.display-xxl}` | 72px | 300 | 1.0 | -1.8px | Hero headline |
| `{typography.display-xl}` | 60px | 300 | 1.05 | -1.2px | Section opener |
| `{typography.display-lg}` | 40px | 300 | 1.1 | -0.8px | Card title / sub-section |
| `{typography.display-md}` | 30px | 300 | 1.1 | -0.3px | Compact card title |
| `{typography.heading-lg}` | 24px | 300 | 1.1 | -0.24px | Pricing tier name |
| `{typography.heading-md}` | 21px | 300 | 1.3 | -0.21px | Section sub-heading |
| `{typography.heading-sm}` | 19px | 300 | 1.4 | 0 | Mini-section label |
...
- **Expanded 1.618 golden-ratio scale on display/heading.** Headline sizes leap between steps far more dramatically than the reference — a bold, confident hierarchy. Tracking scales up proportionally with size, reaching -1.8px at 72px.
...
### Grid & Container
- Marketing pages center in a ~1200px container with the (bolder) gradient mesh extending edge-to-edge above.
...
| 1 | `box-shadow: rgba(0,55,112,0.14) 0 4px 12px, rgba(0,55,112,0.08) 0 2px 4px` | Card lift on white — more pronounced than the reference |
| 2 | `box-shadow: rgba(0,55,112,0.20) 0 20px 48px, rgba(0,55,112,0.12) 0 8px 16px` | Floating panels, dashboard mockup chrome — medium depth with wide spread |
| 3 | Gradient mesh backdrop (saturated) | The brand's primary depth medium — atmospheric color rather than literal shadow |

### Decorative Depth
The gradient mesh IS the depth system, at increased saturation for a louder read. Shadows are pushed further than the reference — wider blur, more spread — for a more dramatic, floating-panel feel appropriate to D2C / marketing-led products.
...
| Token | Value | Use |
|---|---|---|
| `{rounded.xs}` | 8px | Hairline tags, table chrome |
| `{rounded.sm}` | 12px | Form inputs |
| `{rounded.md}` | 16px | Compact cards, alerts |
| `{rounded.lg}` | 24px | Pricing cards, feature cards |
| `{rounded.xl}` | 32px | Dashboard product mockup chrome |
| `{rounded.pill}` | 9999px | All buttons, tag pills |

### Photography Geometry
The brand uses **product UI mockups** more than photography. Dashboard composites render as faux IDE/terminal/dashboard chrome inside `{rounded.lg}` 24px containers with a pronounced `box-shadow`. Real photography appears in customer logo strips and the rare case-study card; treated as inset 4:3 with a soft shadow instead of none.
...
- Background `{colors.canvas}`, padding `{spacing.xxl}`, rounded `{rounded.lg}` 24px, 1px `{colors.hairline}` border, Level 1 shadow.
...
- Background `{colors.canvas-cream}`, text `{colors.ink}`, padding `{spacing.xxl}`, rounded `{rounded.lg}`. Used to break up the indigo / white rhythm with a bolder warmth.

**`card-dashboard-mockup`** — composited dashboard / product UI screenshot.
- Background `{colors.canvas}`, type `{typography.body-tabular}` (with `tnum`), padding `{spacing.xl}` 24px, rounded `{rounded.lg}` 24px, Level 2 shadow. Often contains nested mini-mockups: code preview + dashboard table + chart card.

### Inputs & Forms

**`text-input`** — standard form field.
- Background `{colors.canvas}`, text `{colors.ink}`, type `{typography.body-md}`, padding `{spacing.sm} {spacing.md}` (8px 12px), rounded `{rounded.sm}` 12px, 1px `{colors.hairline-input}` border.
...
**Gradient Mesh Backdrop** — pastel cream → sherbet orange → lavender → saturated indigo → ruby pink stops blurred horizontally across the upper third of the page, louder than the reference. Implemented as SVG or a large background image — not a flat CSS gradient (the real mesh has organic blob shapes).

**Composited Dashboard Mockup** — multi-layer faux-product-UI compositions: an IDE panel on the left, a dashboard table center, a chart card on the right, all rendered at small scale inside `{rounded.lg}` containers with pronounced Level 2 shadows. The composite is the brand's most-photographed feature.
...
- Apply the (saturated) gradient mesh to every marketing hero; bare-canvas heroes feel off-brand.
...
- Let shadows read clearly at Level 1/2 — this variant's confidence comes partly from visible depth.
...
- Don't flatten the shadows back to subtle — this variant's depth is intentionally louder than the reference.
...
| Mobile | < 768px | Pricing 1-up; hamburger nav; display drops 72 → 40px |
...
### Collapsing Strategy
- Display tiers stair-step 72 → 60 → 40 → 30 → 24px through the breakpoints.
- Gradient mesh re-tiles on mobile to preserve the (bolder) wash without disappearing.
...
7. The gradient mesh is non-negotiable on marketing heroes — bare-canvas heroes break the brand, even at increased saturation.
