...
# base: ./DESIGN.md
# brand: Loop
# brand-primary: unchanged (Path B, Stripe-reference primary kept as-is)
# generated: 2026-07-01
version: alpha
name: loop-design-system-balanced
brand: Loop
description: Loop's design system (balanced variant) — the middle-ground reading of the Stripe-inspired reference, kept as close to the original reference values as possible on all 5 brand-variant dimensions (saturation, roundedness, type scale, shadow depth, surface contrast). This is the recommended default for most SaaS / tool-style products.
source: reverse-engineered via VoltAgent/awesome-design-md (design-md/stripe/DESIGN.md), Path B — primary color kept as-is, no brand color swap
...
- **Indigo Deep** (`{colors.primary-deep}` — `#3d2fc0`): A deeper indigo used in gradient mid-stops and as the press-state warmer alternative.
