# ADR-02：in-session 工具只讀，SM-2 真值統一由 async worker 負責
...
`word_progress` 的 SM-2 更新、`mistakes` 記錄，統一由 async worker（`lib/worker.ts`，呼叫 Gemini API）在 session 結束後寫入。

## 理由

- GPT-Realtime-2 是非決定性模型，tool call 可能漏呼或重複呼叫
- 雙重寫入（in-session + async）是競態根源，難以保證冪等
- async worker 從完整 transcript 推導，有完整語境，品質評估更準確
